/**
 *	@file CurvatureApproximation.h
 *	@author Henri Keeble
 *	@brief Declares an evaluator class designed to evaluate the racing line of the target car by approximating optimal path curvature.
*/
...
namespace perfMeasurement
{
	 ...
	 */
	class CurvatureApproximation : public PMEvaluator
	{
	private:
		...
		public:
			...
			PMPoint2D* optimalPoints; /*!< Points on the optimal trajectory */
		...
		const std::string optimalOutputFile; /*!< The file to output optimal points to, for use in plotting. */
		const std::string actualOutputFile;
		const int millisecondsPerOut;
		double timeSinceOut;

		//! Outputs the given optimal points.
		/*!< 
			\param points Optimal points to output.
			\param count The number of optimal points given.
		*/
