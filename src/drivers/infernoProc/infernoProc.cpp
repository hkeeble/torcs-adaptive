/***************************************************************************
    ...
    version              : $Id: infernoProc.cpp,v 1.20.2.1 2008/05/27 22:33:16 berniw Exp $
...
const float infernoCar::CLUTCH_SPEED = 5.0f;				// [m/s]
const float infernoCar::CLUTCH_FULL_MAX_TIME = 2.0f;
...
float getClutch(infernoCar* myc, tCarElt* car);

static const char* botname[BOTS] = {
	"infernoProc 1", "infernoProc 2", "infernoProc 3", "infernoProc 4", "infernoProc 5",
	"infernoProc 6", "infernoProc 7", "infernoProc 8", "infernoProc 9", "infernoProc 10"
};
...
// Module entry point.
extern "C" int infernoProc(tModInfo *modInfo)
{
...
static void initTrack(int index, tTrack* track, void *carHandle, void **carParmHandle, tSituation * situation)
{
	...
	switch (situation->_raceType) {
		case RM_TYPE_PRACTICE:
			snprintf(buffer, BUFSIZE, "drivers/infernoProc/%d/practice/%s", index, trackname);
			break;
		case RM_TYPE_QUALIF:
			snprintf(buffer, BUFSIZE, "drivers/infernoProc/%d/qualifying/%s", index, trackname);
			break;
		case RM_TYPE_RACE:
			snprintf(buffer, BUFSIZE, "drivers/infernoProc/%d/race/%s", index, trackname);
			break;
		default:
			break;
	}
...
static void newRace(int index, tCarElt* car, tSituation *situation)
{
	...
	mycar[index-1] = new infernoCar(myPathfinder, car, situation);
...
static void drive(int index, tCarElt* car, tSituation *situation)
{
	...
	tdble b3;						// Brake value for avoiding high angle of attack.
	tdble steer, targetAngle, shiftaccel;
				// Brake value for control (avoid loosing control).
	tdble b4;				
	infernoCar* myc = mycar[index-1];

	b1 = b2 = b3 = b4 = 0.0;
	shiftaccel = 0.0;

	// Compute myPathfinder according to the situation.
