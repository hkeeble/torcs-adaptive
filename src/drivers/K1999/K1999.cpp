...
//
static const int MaxSegments = 5000;
...
{
 public:
	 CK1999Data(double x1, double x2, double x3, double x4, double x5, const char* psz) :
   WingRInverse(x1),
   TireAccel1(x2),
   MaxBrake(x3),
   SlipLimit(x4),
...
static const char* botname[] = { "K1999-buggy", "K1999-cg-nascar-rwd" };

static CK1999Data data1(0.032, 12.00, 11.00, 2.00, 0.20, botname[0]);
static CK1999Data data2(0.032, 12.00, 11.00, 2.00, 0.20, botname[1]);
...
{ 
 ...
 for (int i = CARS; --i >= 0;)
 {
  ...
  modInfo[i].desc	 = _strdup(botname[i]);
