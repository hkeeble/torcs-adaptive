...
namespace torcsAdaptive
{
	...
	{
		GfuiPrintString("TORCS Adaptive v1.0", cRed.Get(), sFont,
			wWidth - GfuiFontWidth(sFont, "TORCS Adaptive v1.0"), 10, GFUI_ALIGN_HL_VC);

		GfuiPrintString(rTypeName.c_str(), cRed.Get(), mFont, wWidth / 2, wHeight - GfuiFontHeight(mFont), GFUI_ALIGN_HC_VC);

		if (isAdaptive)
		{
			std::string out = "Skill Estimate: " + (skillLevel == NULL_SKILL_LEVEL ? "Gathering data..." : std::to_string(skillLevel));
			GfuiPrintString(out.c_str(), cRed.Get(), mFont, wWidth / 2, wHeight - GfuiFontHeight(mFont) * 2 - 5, GFUI_ALIGN_HC_VC);
		}
	}
